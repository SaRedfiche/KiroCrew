...
class _RecordingRenderer(Renderer):
    ...
    async def on_prompt_choice(self, options, request_id, tool_title="", tool_purpose=""):
...
class TestTurnDriverTranslation:
    ...
    def test_monitor_turn_start_guard_runs_before_acceptance_and_provider_stream(self):
        r = _RecordingRenderer()
        order: list[str] = []
        ...
        _run(p, r, monitor_completion=hook, turn_start_guard=_guard)

        assert order == ["authorize", "guard", "stream"]
        assert hook.accepted

    def test_monitor_turn_start_guard_refusal_never_accepts_or_streams(self):
        r = _RecordingRenderer()
        p = _ScriptedProvider([AcpEvent(kind=EVENT_COMPLETE, stop_reason="end_turn")])
