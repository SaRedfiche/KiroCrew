...
yield-free turn-start boundary; a concurrent stop during channel setup therefore
cannot enter the model. Slack and Discord also run SessionManager's synchronous
shutdown gate in that same yield-free span, before accepting the completion
correlation, so a lease claimed before teardown cannot open a turn after the
shutdown drain snapshot.
...
terminal. If dispatch fails before a turn
