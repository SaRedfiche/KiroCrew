...
| Coordination | Read-only shared view of a project group: the sessions tagged into it (with the coordinator), the coordinator's work items, and file/worktree collisions between sessions; polls live while focused. Always offered — opening it on an untagged session shows an on-ramp that tags the session into a project in one click | Chat right panel → **Coordination** tab; tag a session via its ⋯ / right-click menu → **Project** submenu, or the tab's on-ramp CTA | `components/CoordinationPanel.tsx` (panel + `CoordinationEmptyState` on-ramp), `components/ProjectTagSubmenu.tsx`, `pages/chat/ActivityViewer.tsx`, `pages/chat/SidePanel.tsx` | `project_panel.py`, `chat_folders.py` | `GET /api/projects/{id}/panel`, `GET /api/projects/coordination`, `POST /api/chat/slots/{slot}/project-group` |
