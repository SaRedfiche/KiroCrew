...
conversation remains busy until that cleanup finishes, so a continuation cannot
reseed the registry while release is deleting the same session files. A
...
the manager confirms that the run never registered. A registered run, or a
registration lookup that itself fails, retains its claim and surfaces typed
outcome uncertainty because the child may still be executing. Any failed or
rejected settlement is likewise uncertain; an exact retry is not safe after a
legacy side effect may have occurred.
...
shutdown propagates the failure and retains the command fence, facade, and lease
heartbeat instead of abandoning accepted work in a claimed state.
An active legacy manager record with the requested run id rejects the claimed
command before the keyed executor is invoked, preventing a preassigned id from
overwriting live registries. Direct and lookup responses keep that pre-manager
rejection uncounted so batch reconciliation still supplies its terminal member.
