...
version is marked blocked in its compatibility view and retained for inspection
rather than executed under an older policy; its raw future payload and outer
active intent survive an unrelated store rewrite unchanged. A generic legacy Save
with `active=true` likewise leaves that future outer intent untouched. Its inert
...
explicitly incomplete. GitLab's `requested_changes` detailed merge status maps
to the actionable shared `changes_requested` review decision. Azure optional reviewers
...
turn-start boundary: dashboard after acquiring any unattended background-turn
permit and immediately before appending the wake and entering the runner, and
Slack/Discord in `TurnDriver` immediately before entering the provider stream.
A stop that lands during channel setup or while a dashboard turn is queued behind
the background cap therefore cannot start a stale action turn. Dashboard, Slack,
and Discord receive the same ephemeral,
...
payload. Structured AutoNudge websocket payloads use the owner-only dashboard
...
audit-before-mutation ordering. A legacy create cannot replace a structured
record while its wake is in flight, and the generic service update also fails
closed for structured records.
...
before its Redux upsert or tombstone is applied, so a cached snapshot cannot make
an unseen record replaceable while it refreshes. A live normalized record still
...
deadline or overwrite concurrent configuration. An update parses and
