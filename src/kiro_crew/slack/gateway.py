...
class GatewayOrchestrator:
    ...
    async def _fire_dashboard_nudge(
        ...
        async def _run_dashboard_turn() -> None:
            ...
            # Revalidate only after that permit is held so a stop while queued
            # cannot launch a stale monitor action.
            if completion_hook is not None:
                if not await completion_hook.authorize():
                    return
                _append_nudge()
                completion_hook.mark_accepted()
            await _run_chat(dashboard_state, turn_slot, tagged, **run_kwargs)

        task = spawn_guarded_turn(
            dashboard_state,
            turn_slot,
            dashboard_state.run_background_turn(turn_slot, _run_dashboard_turn()),
        )
