...
def _authorize_upload(profile: str, region: str) -> None:
    """Re-check the local authorization decisions at the moment of upload.

    An archive build can run for minutes inside a worker thread; consent
    withdrawal or the app being disabled during the build must stop the
    upload — the bytes have not left the machine until ``put_file`` runs.
    """
...
def run_snapshot_backup(account: str, profile: str, region: str, bucket: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="kc-backup-") as tmp:
        ...
        _authorize_upload(profile, region)
